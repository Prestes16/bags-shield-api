...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Se for 0 (rede) ou 500, logamos.
    if (status === 0 || status === 500) {
      console.error(`Upstream error (Status: ${e?.status}):`, e.message);
    }
    return res.status(status).json({ success: false, error: String(e?.message ?? 'unknown_error'), meta: { requestId: rid, note: 'Check BAGS_API_KEY and BAGS_API_BASE.' } });
  }
}