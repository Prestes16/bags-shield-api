...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  let payload: CreateTokenInfoRequest;

  try {
    const raw = (req.body ?? {}) as any;
    const obj = typeof raw === "string" ? JSON.parse(raw || "{}") : raw;
    payload = obj as CreateTokenInfoRequest;
  } catch (err) {
    return sendJson(
      res,
      400,
  ...
  if (!payload || typeof payload.name !== "string" || typeof payload.symbol !== "string") {
    return sendJson(
      res,
      400,
      {
        ...
        error: {
          code: "BAD_REQUEST",
          details: {
            message: "Campos obrigatórios ausentes: name, symbol",
          },
        },
        meta: { requestId },
      },
  ...
  let result: BagsResult<CreateTokenInfoResponse>;
