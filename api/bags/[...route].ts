...
function normalizeTokens(payload: any) {
  ...
  const arr = root?.tokens ?? root?.trending ?? root?.items ?? root?.data ?? (Array.isArray(root) ? root : null);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (path === 'token-info') {
      try {
        ...
        trackApiError(e, req, {
          endpoint: '/api/bags/token-info',
          source: 'bags',
          requestId,
          metadata: { path: 'token-info' },
        });
    ...
    if (path === 'create-config') {
      try {
        ...
        trackApiError(e, req, {
          endpoint: '/api/bags/create-config',
          source: 'bags',
          requestId,
          metadata: { path: 'create-config' },
        });
  ...
  if (path === 'ping' || path === '') {
    ...
    if (
      path === '' &&
      rawUrl.includes('/api/bags/') &&
      !rawUrl.endsWith('/api/bags') &&
      !rawUrl.endsWith('/api/bags/')
    ) {
      ...
      if (match && match[1]) {
        ...
        if (fallbackPath === 'trending') {
          ...
          const candidates = [`${BAGS_BASE}/trending`, `${BAGS_BASE}/tokens/trending`, `${BAGS_BASE}/token/trending`];
