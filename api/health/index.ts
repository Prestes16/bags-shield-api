...
/* health: monolithic + CORS + no-store */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  setCors(res);
  if (method !== "GET") { res.setHeader("Allow","GET, OPTIONS"); return res.status(405).json({ ok:false, error:{ code:"METHOD_NOT_ALLOWED", message:"Use GET | OPTIONS" } }); }
