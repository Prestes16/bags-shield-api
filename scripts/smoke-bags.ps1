...
# Default do host: env BS_HOST > padrao
if (-not $HostBase -or -not $HostBase.Trim()) {
  if ($env:BS_HOST -and $env:BS_HOST.Trim()) { $HostBase = $env:BS_HOST.Trim() }
  else { $HostBase = "https://bags-shield-api.vercel.app" }
}
...
function Show([string]$name, $r) {
  $rid = $null
  if ($r.Json -and $r.Json.meta -and $r.Json.meta.requestId) { $rid = $r.Json.meta.requestId }
  $ridShow = if ($rid) { $rid } else { "-" }
  $line = "{0,-24} {1,4}  {2,5}ms  {3}" -f $name, $r.StatusCode, $r.Ms, $ridShow
  if ($r.StatusCode -ge 200 -and $r.StatusCode -lt 300) { Write-Host $line -ForegroundColor Green }
  elseif ($r.StatusCode -ge 400 -and $r.StatusCode -lt 500) { Write-Host $line -ForegroundColor Yellow }
