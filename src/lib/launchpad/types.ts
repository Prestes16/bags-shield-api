...
export interface TokenDraft {
  ...
  /** Descrição do token (opcional, máx 200 caracteres - Metaplex) */
  description?: string;

  /** URL da imagem do token ou data URL para preview (deve ser HTTP/HTTPS válido, anti-SSRF) */
  imageUrl?: string;

  /** Data URL do preview da imagem (apenas em memória, não persiste em draft) */
...
/** Configuração de segurança do launch (Safety Setup) */
export interface SafetyConfig {
  /** Renunciar autoridade de mint (revoke) */
  renounceMint: boolean;
  /** Renunciar autoridade de freeze (revoke) */
  renounceFreeze: boolean;
  /** Meses de bloqueio de liquidez (ex: 12) */
  lpLockMonths: number;
}
...
export interface LaunchConfigDraft {
  ...
  /** Supply inicial do token */
  initialSupply?: number;

  /** Parâmetros de segurança (Safety Setup) */
