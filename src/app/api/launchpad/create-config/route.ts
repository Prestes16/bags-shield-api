...
export async function POST(req: NextRequest) {
  ...
  if (mode === "stub") {
    ...
    const stubResponse = stubCreateConfig(config as LaunchConfigDraft, requestId);
  ...
  try {
    ...
    if (!bagsResult.success) {
      SafeLogger.error("Bags API error", undefined, {
        requestId,
        endpoint: "/api/launchpad/create-config",
        errorCode: ("error" in bagsResult ? bagsResult.error.code : "UNKNOWN"),
      });
      ...
      return NextResponse.json(
        {
          success: false,
          error: {
            code: (("error" in bagsResult && bagsResult.error?.code) ? bagsResult.error.code : "UPSTREAM_ERROR"),
