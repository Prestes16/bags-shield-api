...
// Wallet Management — suporta Phantom, Solflare, Backpack e outras carteiras Solana
const WALLET_PROVIDERS = {
  Phantom: () => window.phantom?.solana || (window.solana && window.solana.isPhantom ? window.solana : null),
  Solflare: () => window.solflare || (window.solana?.isSolflare ? window.solana : null),
  Backpack: () => window.backpack || (window.solana?.isBackpack ? window.solana : null),
  Torus: () => window.torus,
  Glow: () => window.glow,
  // Fallback: carteiras que injetam no padrão window.solana (ex: Exodus, MathWallet)
  Standard: () => (window.solana && typeof window.solana.connect === 'function' ? window.solana : null),
};

function detectProvider(name) {
  if (typeof window === 'undefined') return null;
  const getter = name ? WALLET_PROVIDERS[name] : null;
  if (getter) {
    const p = getter();
    return p && typeof p.connect === 'function' ? p : null;
  }
  // Sem nome: tenta Phantom primeiro, depois Solflare, depois padrão
  return WALLET_PROVIDERS.Phantom() || WALLET_PROVIDERS.Solflare() || WALLET_PROVIDERS.Standard();
}
...
async function connectProvider(name) {
  ...
  if (!provider) {
    throw new Error(`${name} não detectada. Instale a extensão ou use outra carteira.`);
  }
...
async function signTransaction(txBase64) {
  ...
  if (STORE.wallet.type === 'provider') {
    let provider = STORE.wallet.provider;
    if (!provider && STORE.wallet.name) {
      provider = detectProvider(STORE.wallet.name);
      if (provider) STORE.wallet.provider = provider;
    }
    if (provider) {
...
// Status pills — RPC, Mercado, Meteora
async function checkServiceStatus() {
  const updatePill = (el, status) => {
    if (!el) return;
    el.setAttribute('data-status', status || 'checking');
  };

  const rpcEl = $('#status-rpc') || $('#status-rpc-home');
  const mercadoEl = $('#status-mercado') || $('#status-mercado-home');
  const meteoraEl = $('#status-meteora') || $('#status-meteora-home');

  const pills = [rpcEl, mercadoEl, meteoraEl].filter(Boolean);
  pills.forEach((el) => updatePill(el, 'checking'));

  // RPC Solana — rota relativa ao servidor que está servindo a página (3001)
  const rpcUrls = ['/api/rpc/health'];
  let rpcOk = false;
  ...
  [rpcEl, $('#status-rpc-home')].filter(Boolean).forEach((el) => updatePill(el, rpcOk ? 'ok' : 'error'));

  // Mercado — /api/market/summary
  const mercadoUrl = '/api/market/summary?mint=So11111111111111111111111111111111111111112';
  try {
    const r = await fetch(mercadoUrl, { method: 'GET', headers: { Accept: 'application/json' } });
    const ok = r.ok;
    [mercadoEl, $('#status-mercado-home')].filter(Boolean).forEach((el) => updatePill(el, ok ? 'ok' : 'error'));
  } catch {
    [mercadoEl, $('#status-mercado-home')].filter(Boolean).forEach((el) => updatePill(el, 'error'));
  }

  // Meteora — usa /api/health como proxy (backend disponível)
  const meteoraUrl = '/api/health';
  try {
    const r = await fetch(meteoraUrl, { method: 'GET', headers: { Accept: 'application/json' } });
    const ok = r.ok;
    [meteoraEl, $('#status-meteora-home')].filter(Boolean).forEach((el) => updatePill(el, ok ? 'ok' : 'error'));
  } catch {
    [meteoraEl, $('#status-meteora-home')].filter(Boolean).forEach((el) => updatePill(el, 'error'));
  }
}
...
function renderPreviewTransaction(pending) {
  ...
  // Amount
  const amount = pending.type === 'buy' ? `${pending.solAmount} SOL` : `${pending.tokenAmount} TOKENS`;
  ...
  if (scan?.badges && scan.badges.length > 0) {
    ...
    if (badgesEl) {
      badgesEl.innerHTML = scan.badges
        .map((b) => {
...
function renderHistory() {
  ...
  listEl.innerHTML = STORE.history
    .map((entry) => {
...
async function createConfig() {
  ...
  const tipLamports = $('#create-config-tip-lamports')?.value
    ? parseInt($('#create-config-tip-lamports').value)
    : undefined;
...
const ACTIONS = {
  ...
  wallet_connect_solflare: async () => {
    try {
      ...
      alert(`Falha ao conectar Solflare: ${e.message}`);
    }
  },

  wallet_connect_backpack: async () => {
    try {
      await connectProvider('Backpack');
      $('#wallet-modal')?.classList.add('hidden');
    } catch (e) {
      alert(`Falha ao conectar Backpack: ${e.message}`);
    }
  },
...
function boot() {
  ...
  // Verificar status RPC/Mercado/Meteora ao carregar e a cada 30s
  checkServiceStatus();
